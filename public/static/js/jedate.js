...
;(function(window, factory) {
    ...
    jet.extend(jeDatePick.prototype,{
        ...
        dateTemplate : function() {
            ...
            var lyPrev = '<em class="yearprev yprev iconfont" @on="yearBtn(lprev,'+aowArr[0]+')">&#xe67e;</em>',
                // lyNext = '<em class="yearnext ynext iconfont" on="yearBtn(lnext,'+aowArr[2]+')">&#xe944;</em>',
                // ryPrev = '<em class="yearprev yprev iconfont" on="yearBtn(rprev,'+aowArr[3]+')">&#xe611;</em>',
                ryNext = '<em class="yearnext ynext iconfont" @on="yearBtn(rnext,'+aowArr[1]+')">&#xe944;</em>',
                mPrev = '{% if(dlen>2){ %}<em class="monthprev mprev iconfont" @on="monthBtn(mprev,{%=daytit[i].YYYY%}-{%=daytit[i].MM%})">&#xe611;</em>{% } %}',
                mNext = '{% if(dlen>2){ %}<em class="monthnext mnext iconfont" @on="monthBtn(mnext,{%=daytit[i].YYYY%}-{%=daytit[i].MM%})">&#xe609;</em>{% } %}';
            //循环年的模板
