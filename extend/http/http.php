...
class http{
	...
	function getdata($url){
		...
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, $this->ssl_verifytype);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, $this->ssl_verifyhost);
		if($this->PostData){
			curl_setopt($ch, CURLOPT_POSTFIELDS, $this->PostData);
		}
		curl_setopt($ch, CURLOPT_TIMEOUT,$this->TimeOut);
        if($this->ssl_certtype){
            curl_setopt($ch,CURLOPT_SSLCERTTYPE,$this->ssl_certtype);
        }
        if($this->ssl_cert){
            curl_setopt($ch,CURLOPT_SSLCERT, $this->ssl_cert);
        }
        if($this->ssl_keytype){
            curl_setopt($ch,CURLOPT_SSLKEYTYPE,$this->ssl_keytype);
        }
        if($this->ssl_key){
